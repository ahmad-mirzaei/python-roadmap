...
## برنامه ریزی منطق برنامه

قبل از نوشتن کد، می توانیم منطق برنامه را مشخص کنیم:
...
شروع
    ...
    بررسی نام کاربری
        ...
        اگر نام کاربری درست بود:
            بررسی رمز عبور

                اگر رمز عبور درست بود:
                    بررسی وضعیت حساب
...
این ساختار از شرط های تو در تو استفاده می کند، چون هر مرحله به نتیجه مرحله قبل وابسته است.
...
بیایید بررسی کنیم برنامه چگونه کار می کند.

### مرحله ۱ — بررسی نام کاربری

```python
if username == "ahmad":
```

در ابتدا برنامه بررسی می کند که آیا نام کاربری وارد شده با نام کاربری ذخیره شده یکسان است یا نه.

اگر نتیجه شرط `True` باشد، برنامه به مرحله بعد می رود.

اگر نتیجه `False` باشد، دستور زیر اجرا می شود:
...
### مرحله ۲ — بررسی رمز عبور

```python
if password == "python123":
```

بعد از درست بودن نام کاربری، برنامه رمز عبور را بررسی می کند.

اگر رمز عبور درست باشد، برنامه ادامه پیدا می کند.

در غیر این صورت پیام زیر نمایش داده می شود:
...
### مرحله ۳ — بررسی وضعیت حساب

```python
if account_active:
```

برنامه بررسی می کند که آیا حساب کاربری فعال است یا نه.
...
پروژه فعلی کار می کند، اما می توانیم آن را بهتر کنیم.

برای مثال، به جای استفاده از مقادیر ثابت، می توانیم اجازه دهیم کاربر اطلاعات خودش را وارد کند.

نسخه های بعدی می توانند از:

```python
input()
```

برای دریافت اطلاعات زیر استفاده کنند:

- نام کاربری
- رمز عبور

همچنین می توانیم پروژه را با اضافه کردن موارد زیر بهبود دهیم:
...
این بهبودها نیاز به مفاهیم جدیدی دارند که در بخش های آینده یاد می گیریم.
