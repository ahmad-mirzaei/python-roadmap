...
# بخش ۷ — طول Dictionary و شمارش آیتم ها

## مقدمه

یک Dictionary می تواند صفر، یک یا تعداد زیادی جفت Key-Value داشته باشد. با پیچیده تر شدن برنامه ها، معمولاً لازم است بدانیم **Dictionary در حال حاضر چند آیتم دارد**.

Python برای این کار از تابع داخلی `len()` استفاده می کند.

خود عملیات ساده است، اما مهم است دقیقاً بدانیم `len()` چه چیزی را می شمارد، با Dictionary خالی چگونه رفتار می کند و چه تفاوتی میان شمارش Keyها، Valueها و Valueهای منحصر به فرد وجود دارد.

---

## ۱. به دست آوردن تعداد آیتم ها
...
چون Dictionary شامل سه جفت Key-Value است.
...
برای Dictionary، `len()` تعداد **جفت های Key-Value** را برمی گرداند.

---

## ۲. `len()` دقیقاً چه چیزی را می شمارد؟
...
هر آیتم Dictionary شامل یک Key و یک Value متناظر با آن است.

---

## ۳. Dictionary خالی

یک Dictionary خالی هیچ جفت Key-Value ندارد:
...
به همین دلیل `len()` می تواند برای بررسی خالی بودن Dictionary نیز استفاده شود:
...
## ۴. روش Pythonic برای بررسی خالی بودن

کد زیر کاملاً صحیح است:
...
اما Python اجازه می دهد همین مفهوم را ساده تر بنویسیم:
...
Dictionary خالی **Falsy** و Dictionary غیر خالی **Truthy** است.
...
وقتی تنها سوال این است که Dictionary خالی است یا نه، معمولاً روش دوم خواناتر است.

---

## ۵. بررسی وجود آیتم در Dictionary

می توانیم از خود Dictionary در یک شرط استفاده کنیم:
...
این شرط فقط زمانی اجرا می شود که Dictionary غیر خالی باشد.
...
این الگو در برنامه های Python بسیار رایج است.

---

## ۶. تغییر طول هنگام اضافه کردن آیتم

طول Dictionary ثابت نیست.
...
بنابراین طول Dictionary با تغییر ساختار آن تغییر می کند.

---

## ۷. به روز رسانی Value طول را تغییر نمی دهد
...
حالا Value موجود را تغییر می دهیم:
...
چون Key جدیدی ایجاد نکرده ایم. فقط Value مربوط به یک Key موجود را تغییر داده ایم.

پس:

```text
Key جدید             → طول افزایش پیدا می کند
Key موجود + Value جدید → طول ثابت می ماند
```

---

## ۸. اضافه کردن Key جدید طول را افزایش می دهد
...
حالا Dictionary شامل سه Key است:
...
نکته اصلی این است که طول Dictionary بر اساس تعداد **Keyهای منحصر به فرد** تعیین می شود.

---

## ۹. حذف آیتم طول را کاهش می دهد

اگر یک آیتم را حذف کنیم:
...
همین اتفاق با `pop()` نیز رخ می دهد:
...
حالا طول برابر است با:

```text
1
```

پس می توانیم این رابطه را در نظر داشته باشیم:

```text
اضافه کردن Key جدید → طول + 1
حذف Key            → طول - 1
به روز رسانی Value → طول بدون تغییر
```

---

## ۱۰. `clear()` و طول Dictionary

متد `clear()` تمام آیتم ها را حذف می کند:
...
خود Dictionary همچنان وجود دارد، اما دیگر آیتمی داخل آن نیست.

---

## ۱۱. شمارش Keyها به صورت صریح

از آنجا که طول Dictionary تعداد جفت های Key-Value را نشان می دهد، تعداد آن با تعداد Keyها برابر است.
...
اما برای به دست آوردن اندازه Dictionary، معمولاً همین کافی است:

```python
len(data)
```

لازم نیست ابتدا View مربوط به Keyها را دریافت کنیم.
...
چون هر Key یک Value متناظر دارد.

بنابراین در یک Dictionary معمولی:
...
هر سه یک عدد را برمی گردانند.

اما از نظر مفهومی متفاوت هستند:

* `len(data)` → تعداد آیتم های Dictionary
* `len(data.keys())` → تعداد Keyها
* `len(data.values())` → تعداد Valueها

---

## ۱۳. شمارش Valueهای منحصر به فرد

اینجا با یک تفاوت مهم روبه رو می شویم.

Keyهای Dictionary باید منحصر به فرد باشند، اما Valueها می توانند تکراری باشند.
...
چون سه جفت Key-Value داریم.

اما فقط دو **Value منحصر به فرد** داریم:
...
برای شمارش آن ها می توانیم از Set استفاده کنیم:
...
## ۱۴. شمارش آیتم هایی که یک شرط را دارند

گاهی تعداد کل آیتم ها برای ما مهم نیست.

می خواهیم فقط آیتم هایی را بشماریم که یک شرط خاص دارند.
...
فرض کنید می خواهیم تعداد دانش آموزانی را بشماریم که نمره آن ها حداقل `18` است:
...
در اینجا `len(scores)` به ما می گوید چهار دانش آموز داریم، اما Loop فقط Valueهایی را می شمارد که شرط `score >= 18` را داشته باشند.

---

## ۱۵. شمارش شرطی با `sum()`

Python اجازه می دهد همین ایده را به شکل کوتاه تری نیز بنویسیم:
...
این تکنیک زمانی مفید است که با Booleanها و Iteration راحت تر شده باشیم.

---

## ۱۶. شمارش Keyهایی که یک شرط را دارند

می توانیم Keyها را نیز بر اساس یک شرط بشماریم.
...
فرض کنید می خواهیم Keyهایی را بشماریم که حرف `"a"` در نام آن ها وجود دارد:
...
نکته مهم این است که Iteration مستقیم روی Dictionary، Keyها را در اختیار ما قرار می دهد.

---

## ۱۷. استفاده از طول Dictionary در Loop و منطق برنامه

طول Dictionary می تواند برای گزارش وضعیت فعلی داده ها مفید باشد:
...
این قابلیت زمانی مهم می شود که Dictionary در طول اجرای برنامه دائماً تغییر کند.

---

## ۱۸. Dictionary می تواند بزرگ و کوچک شود

فرض کنید Dictionary در طول برنامه تغییر می کند:
...
در هر لحظه، `len()` وضعیت فعلی Dictionary را نشان می دهد.

---

## ۱۹. کاربرد طول Dictionary در برنامه های واقعی

طول Dictionary در موقعیت های مختلف کاربرد دارد.
...
بنابراین `len()` فقط برای نمایش یک عدد نیست؛ می تواند بخشی از منطق تصمیم گیری برنامه نیز باشد.

---

## ۲۰. طول با اندازه داده یکی نیست

`len()` در Dictionary تعداد جفت های Key-Value را نشان می دهد.

اما مشخص نمی کند:

* چند کاراکتر داخل Stringهای آن وجود دارد.
* چند عنصر داخل یک List تو در تو وجود دارد.
* Dictionary چه مقدار حافظه مصرف می کند.
* چند Value منحصر به فرد وجود دارد.
...
چون فقط دو Key در سطح اصلی داریم:
...
تعداد عناصر List داخل `"scores"` روی طول خود Dictionary تأثیری ندارد.

---

## نکات کلیدی

* `len(dictionary)` تعداد جفت های Key-Value را برمی گرداند.
* Dictionary خالی طول `0` دارد.
* به روز رسانی یک Value موجود، طول Dictionary را تغییر نمی دهد.
* اضافه کردن یک Key جدید طول را افزایش می دهد.
* حذف یک Key طول را کاهش می دهد.
* `clear()` طول Dictionary را به `0` می رساند.
* `len(dictionary.keys())` و `len(dictionary.values())` در Dictionary معمولی همان تعداد آیتم ها را نشان می دهند.
* Valueها می توانند تکراری باشند، اما Keyها باید منحصر به فرد باشند.
* برای شمارش Valueهای منحصر به فرد می توان از `len(set(dictionary.values()))` استفاده کرد.
* `len()` فقط آیتم های سطح اصلی Dictionary را می شمارد و به صورت بازگشتی وارد داده های تو در تو نمی شود.
* `not dictionary` روش کوتاهی برای بررسی خالی بودن Dictionary است.
* طول Dictionary می تواند در اعتبار سنجی، گزارش وضعیت و منطق برنامه استفاده شود.
...
با توجه به Dictionary زیر، نتیجه `len(data)` چیست؟
...
آیا طول `data` تغییر می کند؟ چرا؟

## سوال ۳

دو روش متفاوت برای بررسی خالی بودن یک Dictionary بنویسید.
...
1. تعداد کل دانش آموزان را به دست آورد.
2. تعداد دانش آموزانی که حداقل نمره `18` دارند را بشمارد.
3. بررسی کند که Dictionary خالی است یا نه.
4. توضیح دهید چرا `len(scores)` و تعداد دانش آموزانی که حداقل نمره `18` دارند دو مفهوم متفاوت هستند.
...
طول همچنان `2` باقی می ماند، چون Key `"a"` از قبل وجود داشته و فقط Value آن تغییر کرده است.
...
تعداد کل دانش آموزان `4` است، در حالی که تعداد دانش آموزانی که حداقل نمره `18` دارند `2` است.

`len(scores)` تمام جفت های Key-Value را می شمارد.

اما شمارش شرطی فقط Valueهایی را در نظر می گیرد که شرط `score >= 18` را داشته باشند.
