...
# بخش ۱۲ — مرتب سازی لیست ها

## ۱. مقدمه

گاهی لازم است عناصر یک لیست را به ترتیب مشخصی قرار دهیم.

پایتون روش های ساده ای برای مرتب سازی لیست ها در اختیار ما قرار می دهد.
...
با استفاده از `sort()` می توانیم اعداد را از کوچک ترین به بزرگ ترین مرتب کنیم.
...
به صورت پیش فرض، `sort()` از ترتیب صعودی استفاده می کند.

## ۳. مرتب سازی به صورت نزولی

با استفاده از `reverse=True` می توانیم اعداد را از بزرگ ترین به کوچک ترین مرتب کنیم.
...
می توانیم لیستی از رشته ها را نیز مرتب کنیم.
...
رشته ها به ترتیب الفبایی مرتب شده اند.

## ۵. مرتب سازی معکوس رشته ها

می توانیم رشته ها را به ترتیب معکوس مرتب کنیم.
...
## ۶. `sort()` لیست اصلی را تغییر می دهد

نکته مهم این است که `sort()` لیست اصلی را تغییر می دهد.
...
ترتیب قبلی لیست از بین رفته است.

اگر بخواهیم ترتیب اصلی را حفظ کنیم، می توانیم از `sorted()` استفاده کنیم.

## ۷. استفاده از `sorted()`

تابع `sorted()` یک لیست جدید و مرتب شده ایجاد می کند.
...
## ۸. تفاوت `sort()` و `sorted()`

تفاوت اصلی این است:

```text
sort()   → لیست اصلی را تغییر می دهد
sorted() → یک لیست مرتب شده جدید ایجاد می کند
```
...
## ۹. مرتب سازی یک لیست از نمره ها

مرتب سازی برای کار با نمره ها نیز مفید است.
...
می توانیم نمره ها را از بیشترین به کمترین نیز مرتب کنیم.
...
اگر بخواهیم لیست اصلی را بدون تغییر نگه داریم، می توانیم `copy()` و `sort()` را با هم استفاده کنیم.
...
این روش زمانی مفید است که هم نسخه اصلی و هم نسخه مرتب شده را بخواهیم.

## ۱۱. اشتباه های رایج

### اشتباه ۱ — انتظار ایجاد لیست جدید از `sort()`

این کد آن طور که ممکن است انتظار داشته باشیم کار نمی کند:
...
`sort()` لیست اصلی را تغییر می دهد و یک لیست مرتب شده جدید برنمی گرداند.

### اشتباه ۲ — فراموش کردن `reverse=True`

برای مرتب سازی نزولی باید بنویسیم:
...
وقتی می خواهیم خود لیست اصلی مرتب شود، از `sort()` استفاده می کنیم:
...
وقتی می خواهیم یک لیست مرتب شده جدید داشته باشیم:
...
```text
sort()   → لیست اصلی را تغییر می دهد
sorted() → یک لیست جدید ایجاد می کند
reverse=True → ترتیب نزولی
```

# تمرین ها

## تمرین ۱ — مرتب سازی اعداد

یک لیست شامل پنج عدد بسازید و آن ها را از کوچک ترین به بزرگ ترین مرتب کنید.

## تمرین ۲ — مرتب سازی معکوس

یک لیست از اعداد بسازید و آن ها را از بزرگ ترین به کوچک ترین مرتب کنید.

## تمرین ۳ — مرتب سازی میوه ها

یک لیست از میوه ها بسازید و آن ها را به ترتیب الفبایی مرتب کنید.

## تمرین ۴ — حفظ لیست اصلی

یک لیست از اعداد بسازید و با استفاده از `sorted()` یک لیست مرتب شده جدید ایجاد کنید.

هر دو لیست را چاپ کنید.

## تمرین ۵ — مرتب سازی یک کپی

یک لیست از نمره ها بسازید، یک کپی از آن ایجاد کنید و کپی را مرتب کنید.

لیست اصلی و لیست مرتب شده را چاپ کنید.
...
برنامه ای بنویسید که یک لیست از اعداد را از بزرگ ترین به کوچک ترین مرتب کند.

## سوال ۵

برنامه ای بنویسید که یک کپی مرتب شده از یک لیست ایجاد کند، بدون اینکه لیست اصلی تغییر کند.

# چالش

## چالش Python Roadmap

یک برنامه ساده با نام **Score Manager** بسازید.
...
2. یک کپی از نمره ها ایجاد کند.
3. یک نمره جدید از کاربر دریافت کند.
4. ورودی را به عدد صحیح تبدیل کند.
5. نمره جدید را به لیست کپی شده اضافه کند.
6. لیست کپی شده را از کوچک ترین به بزرگ ترین مرتب کند.
7. نمره های اصلی را چاپ کند.
8. نمره های مرتب شده و به روز شده را چاپ کند.
9. بیشترین نمره را چاپ کند.
10. کمترین نمره را چاپ کند.

در این برنامه از مفاهیمی که تا اینجا یاد گرفته اید استفاده کنید، از جمله متغیرها، `input()`، تبدیل نوع، لیست ها، `copy()`، `append()`، `sort()`، `len()` و اندیس گذاری.

---

# بخش ۱۳ — پیدا کردن کمترین، بیشترین و مجموع عناصر لیست

## ۱. مقدمه

پایتون توابع ساده ای برای پیدا کردن اطلاعات کاربردی در یک لیست در اختیار ما قرار می دهد.
...
* `max()`
* `sum()`
* استفاده از این توابع برای لیست های عددی
* ترکیب `sum()` و `len()`
* پیدا کردن میانگین با استفاده از یک لیست

## ۲. پیدا کردن کوچک ترین عنصر با `min()`

تابع `min()` کوچک ترین عنصر یک لیست را برمی گرداند.
...
## ۳. پیدا کردن بزرگ ترین عنصر با `max()`

تابع `max()` بزرگ ترین عنصر یک لیست را برمی گرداند.
...
## ۴. پیدا کردن مجموع با `sum()`

تابع `sum()` تمام اعداد یک لیست را با هم جمع می کند.
...
## ۵. استفاده همزمان از `min()`، `max()` و `sum()`

می توانیم از این توابع در کنار یکدیگر استفاده کنیم.
...
* `min(scores)` کمترین نمره را می دهد.
* `max(scores)` بیشترین نمره را می دهد.
* `sum(scores)` مجموع تمام نمره ها را می دهد.

## ۶. ترکیب `sum()` و `len()`

می توانیم از `sum()` و `len()` برای محاسبه میانگین یک لیست از اعداد استفاده کنیم.

ایده اصلی:

```text
میانگین = مجموع / تعداد عناصر
```
...
## ۷. پیدا کردن اطلاعات مربوط به نمره ها

این توابع هنگام کار با نمره ها بسیار کاربردی هستند.
...
## ۸. پیدا کردن میانگین نمره ها

می توانیم میانگین نمره ها را نیز محاسبه کنیم.
...
## ۹. استفاده از این توابع با یک لیست مرتب شده

این توابع چه لیست مرتب شده باشد و چه مرتب نشده باشد، کار می کنند.
...
لازم نیست قبل از استفاده از این توابع، لیست را مرتب کنیم.

## ۱۰. تفاوت مهم بین این توابع

هر تابع اطلاعات متفاوتی در اختیار ما قرار می دهد:

```text
min() → کوچک ترین عنصر
max() → بزرگ ترین عنصر
sum() → مجموع عناصر عددی
len() → تعداد عناصر
```
...
### اشتباه ۱ — استفاده از `sum()` برای رشته ها

`sum()` برای اعداد استفاده می شود.
...
`sum()` نمی تواند رشته ها را با هم جمع کند.

### اشتباه ۲ — فراموش کردن `len()` هنگام محاسبه میانگین

برای محاسبه میانگین نباید تعداد عناصر را حدس بزنیم.

به جای آن از `len()` استفاده می کنیم:
...
### اشتباه ۳ — مرتب کردن لیست قبل از استفاده از `min()` یا `max()`

برای استفاده از `min()` و `max()` نیازی به مرتب کردن لیست نداریم.
...
برای پیدا کردن کوچک ترین عنصر از `min()` استفاده می کنیم:

```python
smallest = min(numbers)
```

برای پیدا کردن بزرگ ترین عنصر از `max()` استفاده می کنیم:
...
برای شمردن عناصر از `len()` استفاده می کنیم:
...
## تمرین ۱ — کوچک ترین عدد

یک لیست شامل پنج عدد بسازید و با استفاده از `min()` کوچک ترین عدد را پیدا کنید.

## تمرین ۲ — بزرگ ترین عدد

یک لیست شامل پنج عدد بسازید و با استفاده از `max()` بزرگ ترین عدد را پیدا کنید.

## تمرین ۳ — مجموع

یک لیست از اعداد بسازید و با استفاده از `sum()` مجموع آن ها را محاسبه کنید.

## تمرین ۴ — میانگین

یک لیست شامل پنج نمره بسازید و با استفاده از `sum()` و `len()` میانگین آن ها را محاسبه کنید.

## تمرین ۵ — اطلاعات نمره ها

یک لیست از نمره ها بسازید و موارد زیر را چاپ کنید:
...
`min()` چه چیزی را برمی گرداند؟

## سوال ۲

`max()` چه چیزی را برمی گرداند؟
...
برنامه ای بنویسید که با استفاده از `sum()` و `len()` میانگین یک لیست از اعداد را محاسبه کند.

## سوال ۵

چرا قبل از استفاده از `min()` و `max()` نیازی به مرتب کردن لیست نداریم؟

# چالش

## چالش Python Roadmap

یک برنامه ساده با نام **Score Analyzer** بسازید.
...
6. کمترین نمره را چاپ کند.
7. بیشترین نمره را چاپ کند.
8. مجموع نمره ها را چاپ کند.
9. تعداد نمره ها را چاپ کند.
10. میانگین نمره ها را محاسبه و چاپ کند.

در این برنامه از مفاهیمی که تا اینجا یاد گرفته اید، از جمله متغیرها، `input()`، تبدیل نوع، لیست ها، `append()`، `min()`، `max()`، `sum()` و `len()` استفاده کنید.
