...
در این پروژه باید یک برنامه ماشین حساب ساده بسازید.

برنامه باید به کاربر اجازه دهد دو عدد وارد کند و یک عملگر حسابی انتخاب کند.

سپس برنامه باید عملیات انتخاب شده را انجام دهد و نتیجه را نمایش دهد.
...
1. عدد اول را از کاربر دریافت کند.
2. عدد دوم را از کاربر دریافت کند.
3. یک عملگر حسابی از کاربر دریافت کند.
4. عملیات انتخاب شده را انجام دهد.
5. نتیجه را نمایش دهد.

ماشین حساب باید از عملگرهای زیر پشتیبانی کند:
...
بعد از کامل کردن ماشین حساب پایه، قابلیت پشتیبانی از عملگرهای زیر را نیز اضافه کنید:
...
برنامه باید بتواند شرایطی که ممکن است باعث خطا شوند، مانند تقسیم بر صفر، را نیز مدیریت کند.

---

## چالش نهایی

سعی کنید برنامه را طوری طراحی کنید که بتواند چند محاسبه را پشت سر هم انجام دهد، بدون اینکه لازم باشد برنامه را دوباره اجرا کنید.
...
قبل از اینکه سراغ راه حل بروید، **خودتان پروژه را حل کنید**.

مسئله را به قسمت های کوچک تر تقسیم کنید و راه حل خودتان را طراحی کنید.

---

## پروژه کوچک ۲ — بازی حدس عدد

در این پروژه باید یک **بازی حدس عدد** بسازید.

برنامه باید یک عدد مخفی انتخاب کند و کاربر تلاش کند آن عدد را حدس بزند.

بعد از هر حدس، برنامه باید به کاربر بگوید که حدس او:

* بزرگ تر از عدد مورد نظر است.
* کوچک تر از عدد مورد نظر است.
* یا درست است.

بازی باید تا زمانی ادامه پیدا کند که کاربر عدد صحیح را پیدا کند.
...
1. یک عدد مخفی بین `1` تا `100` انتخاب کند.
2. از کاربر یک حدس دریافت کند.
3. حدس کاربر را با عدد مخفی مقایسه کند.
4. مشخص کند حدس کاربر بزرگ تر یا کوچک تر از عدد مخفی است.
5. تا زمانی که کاربر عدد صحیح را پیدا نکرده، به دریافت حدس ادامه دهد.
6. بعد از برنده شدن، تعداد تلاش های کاربر را نمایش دهد.
...
```text
عدد را بین 1 تا 100 حدس بزنید: 50
بزرگ تر است!

عدد را بین 1 تا 100 حدس بزنید: 25
کوچک تر است!

عدد را بین 1 تا 100 حدس بزنید: 37
بزرگ تر است!

عدد را بین 1 تا 100 حدس بزنید: 32
درست است!

شما عدد را در 4 تلاش پیدا کردید.
```
...
بعد از کامل کردن نسخه پایه، بازی را بهتر کنید.

قابلیت انتخاب سطح سختی را اضافه کنید:
...
هر سطح باید تعداد تلاش متفاوتی داشته باشد.
...
اگر تعداد تلاش های کاربر تمام شد، بازی باید پایان پیدا کند و عدد مخفی را نمایش دهد.

---

### چالش نهایی

بعد از کامل کردن بازی، امکان انجام یک بازی جدید را بدون اجرای دوباره برنامه اضافه کنید.

برای مثال:

```text
شما عدد را در 5 تلاش پیدا کردید!

دوباره بازی می کنید؟ بله
```

اگر کاربر `بله` را انتخاب کرد، یک دور جدید از بازی شروع شود.

---

### هدف

قبل از دیدن راه حل، **خودتان پروژه را حل کنید**.

با نوشتن کد شروع نکنید.

ابتدا مسئله را به دقت بررسی کنید و درباره مراحلی که برنامه باید انجام دهد فکر کنید.

---

# بخش ۱۰ — راه حل پروژه ها

## راه حل ۱ — ماشین حساب ساده
...
### چه چیزهایی استفاده کردیم؟

در این پروژه از این مفاهیم استفاده کردیم:
...
شروع
  ↓
انتخاب یک عدد مخفی بین 1 تا 100
  ↓
قرار دادن تعداد تلاش ها روی 0
  ↓
دریافت حدس کاربر
  ↓
...
مقایسه حدس با عدد مخفی
  ├── حدس < عدد مخفی → عدد کوچک تر است
  ├── حدس > عدد مخفی → عدد بزرگ تر است
  └── حدس == عدد مخفی → درست است
                              ↓
                       نمایش تعداد تلاش ها
                              ↓
                            پایان
...
انتخاب عدد مخفی
  ↓
تکرار تا زمانی که تلاش باقی مانده باشد
  ↓
دریافت حدس
  ↓
مقایسه حدس با عدد مخفی
...
بازی حدس عدد چند مفهوم مهم را با هم ترکیب می کند:
...
نکته مهم این نیست که این کد را حفظ کنید.

مهم این است که بتوانید مسئله را به صورت مرحله ای به راه حل تبدیل کنید:

**مسئله → الگوریتم → کد → تست → بهبود**
